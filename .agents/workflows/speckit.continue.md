---
description: "Spec Kit: resume where the last agent stopped, do the next chunk, hand off."
---
...
2. Open `.claude/commands/speckit.continue.md`, read it completely, and follow it exactly. Ignore its YAML front
   matter. Treat any text the user wrote after `/speckit.continue` as `$ARGUMENTS` (scope such as `US1` or
