...
0. **Session start**: follow AGENTS.md section 2 "Session start" (agent id, `status.ps1`, working tree
...
2. Run and parse JSON (`FEATURE_DIR`, `AVAILABLE_DOCS`):
   ...
   ```
3. If `FEATURE_DIR/checklists/` exists, summarise each checklist (total / done / open). If any has open items, show
...
4. Load tasks.md, plan.md, and as available data-model.md, contracts/, research.md, quickstart.md, plus
   `.specify/memory/constitution.md`.
5. Ensure ignore files match the tech in the plan (`.gitignore` for `node_modules/`, `dist/`, `coverage/`,
