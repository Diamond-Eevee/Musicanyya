...
## 2026-09-23 18:40 - claude-opus-5-5 (owner bug report)
- Done: T172, T173 on branch `fix/follow-scroll-after-stop` (from `main` at 19d7410). The owner reported that after Play then Stop, any scroll of the Score was pulled straight back to the cursor
- Cause: `mx-score-view.updateCursor` follow-scrolled on every animation frame whatever the transport phase, so stopped or paused, the view was pinned to the cursor (FR-014 says follow *during playback*). It only showed while the cursor's page was mounted, which is always true in a short piece and why no test caught it. Two more follow bugs turned up while testing: (1) during playback a wheel scroll could land before its `scroll` event, the frame's follow overwrote it and the `followScrolling` flag then swallowed the event, so the scroll was lost and Follow never turned off; (2) follow gave up when the measure's page was not mounted, so ticking Follow from pages away (or a jump back in a long piece) did nothing
- Fix: follow only in Listen `playing` (Practice and Play already followed only a running session or run); any scroll position the view did not set itself counts as the user's, checked in the scroll handler and before each frame's follow; an unmounted measure's page is scrolled into view (page from mounted pages, else one `pageOf` lookup), and the next frame centres the measure. The Follow control is now a checkbox, because the old button showed no on/off state (`aria-pressed` had no style). `data-model.md` section 5 updated
