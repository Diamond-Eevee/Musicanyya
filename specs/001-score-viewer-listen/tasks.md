...
- [~] T097 [P] [US2] WebAudioEngine adapter tests with a fake AudioContext/AudioWorkletNode port (unlock on gesture, state events incl. loadingSound progress, command messages per worklet protocol, latency info, dispose) in `tests/engine/audio/web-audio-engine.test.ts` (claimed: gemini-3.1-pro 2026-09-19)
