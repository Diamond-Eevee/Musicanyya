...
- [x] T170 Fix a notice-report blow-up introduced by T158: `durationToTicks` interpolated the offending text into the notice detail, and `ReportBuilder.add` de-dupes by (code, severity, element, detail) with a linear scan - so a corrupt file produced one entry per distinct bad value, quadratic in time and unbounded in entries (8 000 bad durations: 406 ms and 8 001 entries; a 50 MB file could spin the score worker for minutes and then post a six-figure notice list to the UI). The detail is now a fixed string and `measureLabels` carries the location: 2 entries and 74 ms for the same input. Found by the pre-merge constitution audit
- [x] T171 Stop `<senza-misura>` inheriting the previous measure's nominal length (`src/core/musicxml/build.ts`): setting `mInfo.time = null` is indistinguishable from "no time signature seen yet", so an unmeasured measure following a metered one fell through to the previous nominal and was reported as a length mismatch for not matching a metre it does not have. Found by the pre-merge constitution audit
- [x] T161 **High**: `process()` does not guard `this.inner.processBlock(left, right)` (`score-player.processor.ts`). Anything thrown by spessasynth's `process` or by the dispatch math escapes `process()` and permanently kills the processor - silent output for the rest of the session with no diagnostic. Wrap it, set a `faulted` flag, return `true`, and post the error **once** from outside the hot path, guarded by the flag so it cannot post per quantum. Needs an owner decision on what the UI shows when the processor faults mid-session.
  **Resolved 2026-09-25 (claude-sonnet-5)**: guarded in the testable factory (`processBlock` wraps a new
  `processBlockInner` in try/catch, plus a class-level backstop in `process()` itself), posts a new
  `status: processorFaulted` message once. UI wording chosen without a formal owner sign-off (a bug fix
  restoring a diagnostic where there was none, not new user-facing scope) - see implementation-log.md; owner can
  revise `en.notices.processorFaulted`'s text freely, it is not load-bearing elsewhere.
...
- [ ] T169 Track the percussion note-count gap (`tests/fixtures/musicxml/spec-examples/tutorial-percussion.musicxml`): the Score model holds 36 notes but Verovio draws 32 `g.note` elements on its single page, the only fixture anywhere where the two disagree. Principle III wants Note ID = SVG id for every playable note, and unpitched percussion is listed as supported in `SUPPORT_MATRIX`, so either the four extra notes should be engraved or they should not be in the model. Currently documented in that folder's README and deliberately not asserted by `tests/e2e/real-scores.spec.ts`; found by the pre-merge constitution audit
...
## Follow fixes (2026-09-23, branch `fix/follow-scroll-after-stop`)

Reported by the owner on Fur Elise: after Play then Stop, any scroll was pulled straight back to the cursor.

- [x] T172 Follow only while the cursor moves (FR-014 "during playback"): `mx-score-view` follow-scrolled on every frame even when stopped or paused, so the Score could not be browsed after Stop. A user scroll during playback was also lost when the animation frame ran before its `scroll` event (the follow overwrote it and a flag swallowed the event); the view now treats any scroll position it did not set itself as the user's. The Follow control becomes a checkbox so its on/off state is visible (it was a button whose `aria-pressed` had no visual style). Tests: `tests/e2e/us2-listen.spec.ts` "Follow (FR-014)", `tests/ui/transport.test.ts`
- [x] T173 Follow a measure whose page is not mounted: ticking Follow after scrolling pages away (or a jump back, e.g. D.C., in a long piece) did nothing, because follow gave up when the measure's element was missing. The view now scrolls to that measure's page (known from mounted pages, else asked of Verovio once), which mounts it; the next frame centres the measure. Applies to Listen, Practice and Play. Test: the same e2e ("Ticking Follow brings the playing measure back into view, even from pages away"); it fails with this path disabled