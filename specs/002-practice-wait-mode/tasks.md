...
- [x] T020 [US1] RT review with `rt-audio-reviewer` (`.claude/agents/rt-audio-reviewer.md`) of T018: confirm that
  ...
  is checked)
- [x] T053 [US1] Found while wiring US2: `createStore` in `src/ui/state/store.ts` compared any two `Map`s or `Set`s as
...
- [x] T039 [P] [US4] `tests/ui/practice-help.test.ts`: the expected key is lit on the on-screen keyboard with its
...
- [x] T041 [US4] `src/ui/elements/mx-practice-help.ts`, the expected-key highlight in
