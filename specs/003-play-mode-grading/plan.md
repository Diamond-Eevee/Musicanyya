...
**Branch**: `003-play-mode-grading` | **Date**: 2026-09-20 (clarified 2026-09-20) | **Spec**: [spec.md](spec.md)
...
Metronome. Both carry a mandatory `rt-audio-reviewer` review. Nothing else on the RT path changes: the Metronome is
scheduled data, and grading never runs while the musician plays
...
|---|---|---|---|
| I | Real-Time Safety (Web and Native) | New code in AudioWorklet/plugin callbacks allocation-, await- and log-free? Sounds scheduled ahead on the audio clock (no timers)? Heavy work off the main thread? | [x] The Metronome and the count-in are **scheduled events**, not new RT code (R-02): they ride the existing dispatch path, so no timer and no second scheduler exists anywhere in this feature. The two RT changes are sub-block rendering over the pre-allocated `splits` array and a `channelVolume` message handled in `port.onmessage`; neither allocates, awaits, logs or throws, and both get an `rt-audio-reviewer` review. Grading runs in `grade.worker.ts` (R-08), never on the main thread and never during a run (FR-026). Recording a MIDI message is an array push on the main thread, off the RT path |
...
|   |-- grade/                 # NEW, pure TS
|   |   |-- types.ts           # ExpectedNote, NoteResult, ExtraNote, Grade, GradeSummary, ResultReason
|   |   |-- expected.ts        # buildExpectedNotes: flattens 002's buildExpectedEvents, slices to the range
...
|   `-- defaults.ts            # + the Metronome, count-in, strictness and calibration constants
|-- engine/
...
### Open owner decisions (AGENTS.md section 7)

Four findings change user-visible behaviour, a success criterion or MusicXML scope, so they are **not** decided
here. None of them blocks `/speckit.tasks`: each has a recommended default that the tasks can assume, and each is
cheap to reverse before implementation reaches it.

| # | Question | Recommendation | Cost if deferred |
|---|---|---|---|
| D-1 | **Ornaments** (R-17). A correctly played trill produces presses no expected note can claim, so FR-018 reports them as extra and an excellent trill becomes the worst-scoring thing in the piece - while the spec's own edge case says playing an ornament "is neutral". Closing it needs `<ornaments>`, `<trill-mark>` and `<tremolo>` parsing, which is new MusicXML scope | Parse the ornament elements and treat presses of the ornamented note's pitch and its diatonic neighbours, within its written duration, as played-along | Any Score with a trill grades unfairly. The interim is to mark those measures the way unreliable ones are marked |
| D-2 | **Written arpeggios** (R-17). `<arpeggiate>` means the chord *should* be rolled, so FR-022 marks a correct performance late. Also new MusicXML scope | Parse `<arpeggiate>` and use `PLAY_ARPEGGIO_SPREAD_BEATS = 0.5` in place of the chord spread there | Rolled chords that are written as rolled read as late. Less damaging than D-1 |
| D-3 | **SC-015 says the live marks and the Grade agree for 100% of the notes they cover**, but FR-011a exists precisely because they can disagree, and with the order-preserving matcher they provably can: a live "correct" can be overturned when a later press turns out to be that note's match | Soften SC-015 to a measured rate over the reference fixtures, with the disagreement mechanism documented - the live marker stays a cheap pitch test | Must be settled before `/speckit.tasks`: it decides whether the live marker is a simple pitch test or has to mirror the matcher, which is a different task list |
| D-4 | **FR-003's "nothing played during the count-in is graded"**, read literally, makes an early first note impossible - and playing the first note slightly early is the commonest beginner tendency there is (R-16) | Scope the exclusion to the count-in *minus the first expected note's early claim window*; the design above assumes this | Every run's first note can only be late or missed |

D-4 is already designed in as the recommendation because the alternative is a defect rather than a choice; if the
owner prefers the literal reading, one constant and one fixture change.

### Scope note for `/speckit.tasks`

The Latency profile and its calibration (R-05, FR-034) are a bigger piece of work than the spec's Assumptions
suggest, because feature 001 supplies neither. They are **not** on the P1 path: US1 grades correctly with an
assumed profile that the Grade labels honestly. The tasks list should therefore put the calibration with US2,
where FR-034 lives, and keep US1 free of it.
