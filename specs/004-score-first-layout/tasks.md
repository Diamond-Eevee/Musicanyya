...
**T004 verdict (2026-09-21, claude-sonnet-5)** - none is invalidated or superseded; all five stay open
on their own features:

| Task | Verdict | Note |
|---|---|---|
| 001 T138 (MusicXML mutation fuzz) | still valid | `src/core` only; this feature does not touch it |
| 001 T139 (justify `any` in workers/worklet) | still valid | shares `verovio.worker.ts` with T030 but not the lines involved (T139 covers `catch (err: any)` and `toolkit: any`); do whichever lands second on top of the other |
| 001 T141 (Electron playback e2e) | still valid | must use the new bar selectors once T037 lands; independent of T111, which only asserts layout |
| 003 T082 (manual check on a real MIDI keyboard) | still valid, **run after 004 merges** | its script names old panel positions; re-read it against the Setup popup and slim bar first |
| 003 T110 (integer ticks in grading) | still valid | `src/core` only |

---

## Phase 1: Setup

- [x] T001 Pin Verovio's page-unit relation before any UI work (research R-2, `score-layout.md` section 2
...
- [x] T002 If T001's measurements differ from the assumed rule, correct
...
- [x] T003 [P] Add `SCORE_SCALE_MIN` (50), `SCORE_SCALE_MAX` (200), `SCORE_SCALE_DEFAULT` (100),
      ...
      (T001 confirmed both page-unit bounds are accepted verbatim by Verovio).
- [x] T004 [P] Re-check the five inherited open tasks (001 T138/T139/T141, 003 T082/T110) against the
...
- [x] T020 [P] [US1] `tests/e2e/us1-layout.spec.ts`: at 1280x720, 1366x768, 1600x900, 1920x1080 and
      ...
      space (SC-006).
- [x] T021 [P] [US1] `tests/e2e/us1-layout.spec.ts` (same file, second describe): at 1920x1080 at least
      two systems of the two-staff fixture `tests/fixtures/musicxml/large-score.musicxml` (500 measures,
      generated by `tests/tools/gen-large-score.ts`) are fully inside the viewport without scrolling
      (SC-002). Corrected 2026-09-21: the fixture first named here,
      `.../chords/c-major-scale-and-chords.musicxml`, has only 2 measures and one system.
- [x] T022 [P] [US1] `tests/ui/score-view-fit.test.ts`: `mx-score-view` derives its `LayoutOptions` from
...
- [x] T023 [P] [US1] `tests/ui/size-controls.test.ts`: `mx-size-controls` renders larger / smaller /
...
- [x] T024 [P] [US1] `tests/ui/shortcuts.test.ts`: the existing bare `+` / `=` and `-` / `_` keys **and**
...
- [x] T025 [P] [US1] `tests/ui/empty-state.test.ts`: with no Score loaded the Score area shows one
...
- [x] T026 [US1] Rewrite `src/ui/elements/mx-app.ts` to the regions of `ui-shell.md` section 1: a single
...
- [x] T027 [US1] Rewrite `src/ui/styles/layout.css`: bar height token at most 48 px at 100% scaling,
...
- [x] T028 [US1] `src/ui/elements/mx-score-view.ts`: observe the scroll container with a
...
- [x] T029 [US1] `src/ui/score/pages.ts` and `mx-score-view.applyPageCount()`: take the page height from
...
- [x] T030 [US1] `src/workers/verovio.worker.ts`: set `adjustPageHeight` to `0` (T001: required for a
      dictated height) for both `load` and `relayout`, leaving the message shape unchanged (the worker
      contract stays backward compatible). Test first: add a worker-path case to
      `tests/verovio/page-units.test.ts` asserting a rendered page's outer height equals
      `pageHeight * scale / 100`, and see it fail before changing the worker.
- [x] T031 [P] [US1] `src/ui/elements/mx-size-controls.ts` - larger / smaller / reset bound to
      `viewState` (makes T023 pass; depends on T013).
- [x] T032 [US1] `src/ui/shortcuts.ts`: move the bare `+` / `=` / `-` / `_` Score-size keys here from
...
- [x] T033 [US1] `src/ui/elements/mx-drop-zone.ts` plus the empty state in `mx-app`: the invitation fills
      the Score area and accepts a drop anywhere in it (makes T025 pass).
- [x] T034 [US1] `src/app/session.ts`: mount the Score view, mode switch, transport, open button, size
      ...
      T032).
- [x] T035 [US1] `src/ui/styles/score.css`: page sizing without the 1200 px cap, insets consumed from
      `--mx-inset-*`, no horizontal overflow at any `scale` (G-1).
- [x] T036 [US1] Update the existing unit tests whose DOM assumptions moved -
...
- [x] T037 [US1] Update the existing e2e specs' selectors for the new regions -
...
- [x] T038 [US1] `tests/e2e/us1-open-view.spec.ts:35`: the persisted-zoom assertion reads
...
- [x] T039 [US1] `tests/e2e/us1-layout.spec.ts` (same file as T020/T021): a malformed MusicXML file
...
- [x] T040 [P] [US2] `tests/e2e/us2-panels.spec.ts`: every menu entry opens its panel over the Score;
...
- [x] T041 [P] [US2] `tests/e2e/us2-panels.spec.ts` (same file): with a panel open, starting Listen,
...
- [x] T042 [P] [US2] `tests/ui/escape-precedence.test.ts`: with a panel open Escape closes the panel and
      does **not** stop the transport; with no panel open Escape stops it (research R-4, `ui-shell.md`
      section 4).
- [x] T043 [P] [US2] `tests/ui/panel-host.test.ts`: `mountPanels()` (`src/ui/layout/panel-host.ts`, the one
      function `session.ts` calls to mount its ten tools - `session.ts` itself cannot be unit-tested, it
      creates Workers) wraps all ten `PanelId` tools in `mx-panel` inside `#panel-host`, each with the right
      `data-panel` and heading, and none of them is a child of the bar or of `mx-score-view`.
- [x] T044 [P] [US2] `tests/ui/panel-keyboard.test.ts`: every control inside an open panel is in the tab
      order, and the panel is dismissible without a mouse (FR-005, Acceptance 2.5).

### Implementation

- [x] T045 [US2] `src/app/session.ts` and new `src/ui/layout/panel-host.ts` (`mountPanels()`): re-parent `mx-recent-list`, `mx-attempts-list`, `mx-midi-panel`,
      ...
      T017, T034).
- [x] T046 [US2] `src/ui/shortcuts.ts`: Escape precedence - close the open panel and return, otherwise
      stop the transport (makes T042 pass; depends on T013).
- [x] T047 [US2] Wire the four `mx-menu` instances in the bar to `menu-model.ts`, disabling entries that
      cannot apply (no Score loaded) rather than hiding them (depends on T018, T026).
- [x] T048 [US2] Call `viewState.closeForRun()` at the single place each run starts in
      `src/app/session.ts` (Listen, Practice, Play) so FR-006 has one call site (makes T041 pass; depends
      on T013).
- [x] T049 [US2] Verify and, if needed, enforce that opening or closing a panel changes no layout input
...
- [x] T050 [US2] Add the Escape-precedence rule and the new shortcuts to the shortcut list in
