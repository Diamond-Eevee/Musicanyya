...
## 2026-09-24 23:55 - claude-opus-5.5 (implement: Phase 7 Polish, T082-T088 + T102-T105; final entry)
- Session start: `pnpm test` `Tests 2115 passed (2115)`, `pnpm lint` 0 errors, as the last entry says.
- Owner decisions (2026-09-24, all "recommended"): feature-005 data-model/research no longer call the close-position cadence "perfect" (FR-004 itself never did); Satie keeps its chords on the lower staff, documented (T102); Satie/Clementi provenance follows Für Elise (T103); Clementi relabelled as the plain conversion (T104); Chopin No. 4 is the plain conversion with 9 notes re-staffed (T105).
- Done:
  - T082: README, reference R7 and AGENTS.md "Seeing the app" name `pnpm library:fidelity` (`--check`, `--item`) and `pnpm library:convert-ly`; screenshot examples use `pnpm screenshot --item` (the script also drops a leading `--` since 116f519). `public/library/README.md` gains "Audit" with "a replaced item is converted, never hand-fixed". AGENTS.md 11,670 characters.
  - T084: sweep test green, so every item's notices equal `expected.notices`; only Für Elise has one (`measureLengthMismatch`, explained in limitations). Converted files used `<slur>`, `<tuplet>`, `<octave-shift>`, `<pedal>`, `<rights>`, `<source>`, which were not listed: added to `SUPPORT_MATRIX` and `docs/musicxml-support.md` (the sync test failed first, then passed).
  - T083: every one of the 11 sources is now listed exactly once: 6 in the library list (Für Elise, Chopin No. 4, Chopin No. 20, Burgmüller No. 2, Satie, Clementi), 5 under "Reference sources". The check found:
    - T102: de2d579 (T041, antigravity) had edited the committed Satie `.ly` (moved the accompaniment voice to the bass staff) and re-pinned its hash, against contract `source-manifest.md` §1 / FR-016. Restored to the downloaded bytes (sha256 `420a5224...`, from c3df327); the Satie record still gives 0 differences on every aspect (staff is not an aspect).
    - T103: Satie and Clementi sidecars still said `authored`/CC0 with pre-conversion `basedOn` text; now `downloaded`/public-domain; stale `arranger` removed (also Burgmüller No. 2).
    - T104: Clementi held the whole first movement (38 bars, both repeats) but was labelled "exposition, bars 1-15", and its tempo had been hand-edited to 144 after conversion. Now exactly the `convert-ly` + `engrave` output (tempo 156, half = 78, from the source MIDI); the diff to the old file is only the title and that tempo. Identity golden: only its `scheduleDigest` changed.
  - T085 (manual verification), each step run:
    - US1.1 `--item chopin-prelude-op28-no4`: `0 differences (expected 0)`; US1.2 planted B3->C4 in the pickup: exactly `bar 0, beat 0: pitch C4, source B3`, record fails; US1.4 report row: Peters, 1879, Mutopia 468, mechanical, 0.
    - US2.1 Für Elise 16-bar: four melody checks with 1/2/4/1 counted differences = expected, rhythm differences allowed by departures; US2.2 `meta.departures` lists six departures naming bars.
    - US3.1 B-flat minor `0 differences`; US3.2 planted F##4->G4: `bar 2: right hand, chord 3: expected F##, found G4 (spelling)`.
    - US4.1 report 58 items, levels 7/2/7 against 7/5/5; US4.2 `--check` exit 0; US4.3 Bach BWV 846 `--item` 0 = report 0.
    - Screenshots looked at (`pnpm screenshot --item ... --full`; notices none, console errors none for all): Satie - "Gymnopedie No. 1", Erik Satie, two sharps, 3/4, "Lent et douloureux", low dotted-half bass, chords on the lower staff, no "our own close"; Ode to Joy - "(theme, arranged for beginners)", "arr. Musicanyya practice material"; Clementi - "(1st movement)", "Spiritoso", beams drawn; Burgmüller No. 2 - "l'Arabesque", no arranger line, beams drawn; Chopin No. 4 - "Largo", pedal marks engraved. The app shows one system, so the last pages were rendered with Verovio to `tests/.generated/t085/` and looked at: Chopin bars 22-25 (right-hand chords on the upper staff, left-hand B1-B2 and E1-E2 octaves) and Satie's second ending (4 bars).
    - `quickstart.md` Manual verification rewritten to what the tools print and to the Advanced ids (US1.1, US1.2, US1.3 end of piece, US2.1, US3.2, App still works).
    - T105, found here: Chopin No. 4's "arrangement (playable hand-span fixes)" had raised the left-hand B1 of the octaves in bars 17 and 24 to B2 - unisons on one key - and its `differenceNotes`/`departures` named a "B4 to B3 in bar 9" that was never made. Measured: the plain conversion fails only criterion 16, in bars 24-25 (the right hand's cross-staff chords are counted with the left hand because the span is per printed staff). Now the plain conversion with those 9 notes on the upper staff: claim `original`, `arrangement: false`, 0 differences on every aspect; the source's 48 pedal marks are back (limitation "written pedal is not played"). Identity golden re-captured for that item only (only its lines changed). Anyone who practised it has new Note IDs; the item id is unchanged.
  - T086 `constitution-auditor` (subagent, ran): COMPLIANT WITH NOTES, no CRITICAL/HIGH. LOW: (1) `http-catalog.ts:147` returns `tooLarge` for an oversized cached copy instead of trying the network - confirmed, an edge case (a copy is only cached after the same size check); offered as a separate task, not changed here; (2) the notices name only some source file names (optional); (3) T027-T030, T095, T031-T037, T045 have no log entry of their own (already noted in this log).
  - `music-domain-expert` review (subagent, ran) of the Satie/Clementi relabels. Applied: Satie's source already has bars 19-31 on the lower staff (verified in the `.ly`: 18 bars before `\change Staff=bass`), no written pedal and no *mp* (tag `pedal`, "(pp/p/mp/f)" and the pedal limitation removed); Clementi `trains` covers the whole movement; tempo stated as half = 78. Not applied: "held G in the bass, bars 16-23" (the `.ly` left hand moves F, E-flat, B-C, G); the Satie title accent ("Gymnopedie" kept, as in the file's `<work-title>`); Clementi's level (it computes to Advanced; D-3).
- Outcomes (report): verified 36, verified (visual) 3, fixed 11, replaced 4 (Satie, Burgmüller No. 2, Clementi, Chopin No. 4), relabelled 3, removed 1; total 58.
- Level counts (pieces): Beginner 7 (min 7), **Intermediate 2 (min 5, short by 3 - FR-022, reported, not filled)**, Advanced 7 (min 5).
- Gate (T087):
  - `pnpm lint`: 0 errors (`Found 284 warnings`, `Found 13 infos`);
  - `pnpm typecheck`: exit 0;
  - `pnpm test`: `Test Files 184 passed (184)`, `Tests 2115 passed (2115)`;
  - `pnpm library:fidelity`: `58 records, 0 failed`, 0.8 s wall time (plan goal < 30 s, analyze A8); `--check`: `docs/library-audit.md is up to date`, exit 0;
  - `pnpm test:e2e`, full run: exit 1: `1 failed` (`[electron] library.spec.ts:175:3`, the known-flaky Electron launch, reference R7), `65 skipped`, `294 passed (3.8m)`; that spec alone: `1 passed (3.6s)`, exit 0. **As at every 007 checkpoint, e2e was green only on the re-run of that one spec.**
- Problems / open questions (owner-facing):
  - Intermediate is 3 pieces short (FR-022, D-3).
  - Criterion 16 measures chord span per printed staff, not per hand; a later feature could measure it per voice home staff (research.md, T105 alternatives).
  - Satie's file cannot be regenerated by `convert-ly` from its restored source (the staff placement); this is recorded in its sidecar and audit note.
- Handoff: next = nothing in scope; **ready to merge when the owner agrees** (every task ticked, constitution review passed). Not pushed. Tree clean after this commit.
