...
- **Clementi Sonatina Op. 36 No. 1 (Mvt 1)**: The tempo marking 144 is editorial and thus an arrangement departure from the original. Its `arrangement` flag is `false` after converting it faithfully, but we adjusted the tempo down to 144 in the MusicXML and declared it as a limitation, while the source's notes remain faithful. The item has been moved to the `advanced` level due to computed metrics.
  **Superseded 2026-09-24 (T104, owner decision)**: the converted file holds the whole first movement (38 bars, both repeats), not the exposition, and the tempo edit was a hand edit of converted output. The item is now exactly what `library:convert-ly` + `library:engrave` produce (tempo 156 from the source MIDI), titled "Sonatina in C major, Op. 36 No. 1 (1st movement)", claim `original`, no tempo limitation. Rationale: a replaced item is converted, never hand-fixed; at Advanced the tempo cap no longer needs 144. Alternative rejected: a bar-range option for `convert-ly` to restore a bars 1-15 excerpt at Intermediate (more tooling, delays the merge).
- **Chopin Op. 28 No. 4 (T105, owner decision 2026-09-24)**: the earlier "arrangement (playable hand-span fixes)" raised the left-hand B1 of the octaves in bars 17 and 24 to B2 (unisons) and described edits it did not contain. Measured: the plain conversion fails only criterion 16, in bars 24-25, because the span is taken per printed staff and the right hand's cross-staff chords sit on the lower staff. Decision: plain conversion with only those 9 notes printed on the upper staff; claim `original`, 0 differences. Alternatives rejected: measuring criterion 16 per hand (voice home staff) - more correct, but a change to feature-005 level facts for every item, left for later; keeping the arrangement with corrected notes (the unisons are unplayable).
- **Satie Gymnopédie No. 1, staff of the accompaniment (T102, owner decision 2026-09-24)**: the committed `.ly` had been edited to move the `middle` voice to the bass staff; it is restored to its downloaded bytes. The item keeps the chords on the lower staff (left hand in Practice and Play; staff is not a compared aspect) and says so in its sidecar note, audit note and the notices entry. Alternatives rejected: re-converting as printed (the chords would count as right hand); a recorded voice-to-staff option for `convert-ly` (more tooling).
