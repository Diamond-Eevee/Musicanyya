...
| VI | Musician-First Feedback | Colour + shape: correct = the printed head, wrong = an added disc at another position (owner decision 2026-09-25 on greyscale limit, R-03); held-over has an upward chevron, skipped a skip chevron (analyze A1); a disc stays in the current note's column and may lie over a written head while its key is held (FR-006, R-09, owner decision 2026-09-25); band sits behind the notes; both layers switchable | PASS with an owner-accepted exception (disc over the current note's head: Complexity Tracking, accepted 2026-09-25); the rest owner-approved interpretation, recorded in spec Clarifications |
...
| **Exception to Constitution VI** ("Overlays ... MUST NOT hide the notes they refer to"): a red disc partly covers a written notehead | Owner decision 2026-09-25 after using the feature (spec Clarifications, FR-006): a disc moved right of the head read as a mistake on the next note; in the column it reads as "this note, wrong key", as in Piano Marvel. The disc *is* feedback on the current note, so this is an exception, not a reading. Bounds: (1) only while the key is held (FR-005); (2) the disc is at most `DISC_SIZE_RATIO` (0.85) of a head: at a second it covers about 40 % of the head's height, at the same position the head shows as a rim around it, so the head's staff position stays readable; (3) the covered heads are those of the current event, plus, for the upper of two discs a second apart (one slot right), a head Verovio displaced to that side or the current note's augmentation dots; (4) state chevrons are drawn on top of discs. **Accepted by the owner 2026-09-25** (no constitution amendment; the alternatives were a MINOR amendment of principle VI, or redrawing the head's outline over the disc) | Moving the disc aside (R-09 before 2026-09-25: rejected by the owner); a see-through disc over the head (offered, not chosen); redrawing the written head's outline over the disc (keeps the letter of VI; possible if the owner prefers it, the notehead glyph is already harvested) |
