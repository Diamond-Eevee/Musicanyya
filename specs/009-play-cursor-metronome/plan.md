...
| During a Play run a key press has no visual response on the Score, and none at all while the on-screen piano is off (its default); the constitution's budget asks for "visual feedback for a played note: <= 50 ms" (Constitution II/VI) | Owner decision 2026-09-25 (owner review): "It's a grade and should be shown after" - no mark during a run (FR-027); the audit's follow-up question was answered "accept". The key still sounds at once through the live channel (003 FR-006); the on-screen piano shows it when switched on; Play is judged afterwards by definition. Practice and Listen are unchanged | A neutral "key is down" mark on the Score or an automatic on-screen piano in Play (new behaviour the owner did not ask for) |
