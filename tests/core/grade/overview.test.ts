...
describe('Measure overview (FR-032, AS-1.9)', () => {
  it('keys by measure pass, so each occurrence of a repeated measure is counted separately', () => {
    const expected: ExpectedNote[] = [
      { index: 0, noteIds: ['n0'], key: 60, onsetTick: 0, measureIndex: 0, passIndex: 0, chordSize: 1, arpeggiated: false },
      { index: 1, noteIds: ['n1'], key: 62, onsetTick: 480, measureIndex: 0, passIndex: 0, chordSize: 1, arpeggiated: false },
      { index: 2, noteIds: ['n2'], key: 60, onsetTick: 960, measureIndex: 0, passIndex: 1, chordSize: 1, arpeggiated: false },
    ];
    const results: NoteResult[] = [
      { expectedIndex: 0, noteIds: ['n0'], pitch: 'correct', timing: 'onTime', playedKey: 60, deltaTicks: 0, deltaMs: 0, reason: { code: 'correctOnTime', expectedKey: null, playedKey: null, octaveDelta: null, deltaMs: null } },
      { expectedIndex: 1, noteIds: ['n1'], pitch: 'missed', timing: null, playedKey: null, deltaTicks: null, deltaMs: null, reason: { code: 'missedNothingPlayed', expectedKey: null, playedKey: null, octaveDelta: null, deltaMs: null } },
      { expectedIndex: 2, noteIds: ['n2'], pitch: 'correct', timing: 'onTime', playedKey: 60, deltaTicks: 0, deltaMs: 0, reason: { code: 'correctOnTime', expectedKey: null, playedKey: null, octaveDelta: null, deltaMs: null } },
    ];
  ...
  it('puts measures 3 and 7 worst in the known-mistake fixture', () => {
    ...
    const input = buildGradeInput(fixture.scoreFixture, fixture.settings.selection!, fixture.log, { settings: fixture.settings });
