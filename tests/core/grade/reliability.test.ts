...
describe('Reliability warnings (FR-015, AS-2.5)', () => {
  it('marks measure passes overlapping reliability events as unreliable, leaving the rest usable', () => {
    const REASON: ResultReason = { code: 'correctOnTime', expectedKey: null, playedKey: null, octaveDelta: null, deltaMs: null };
    ...
    const expected = passes.map((p, i) => ({ index: i, noteIds: [`n${i}`], key: 60, onsetTick: p.startTick, measureIndex: p.measureIndex, passIndex: i, chordSize: 1, arpeggiated: false } as ExpectedNote));
    const results = expected.map(e => ({ expectedIndex: e.index, noteIds: e.noteIds, pitch: 'correct', timing: 'onTime', playedKey: 60, deltaTicks: 0, deltaMs: 0, reason: REASON } as NoteResult));
    ...
    const { measures, reliability: warnings } = computeSummary(expected, results, [], reliability, passes, expected.map(() => false));
