...
import type { HandSelection, PracticeSession, PracticeEffect } from '../../../src/core/practice/types.js';

function playSession(session: PracticeSession, inputs: string[]): { session: PracticeSession; effects: PracticeEffect[] } {
...
describe('matcher', () => {
  ...
  it('accepts chords only when all required keys are held, with correctSoFar', () => {
    ...
    let step1 = playSession(session, ['on:43@100']);
    expect(step1.session.index).toBe(0);
    expect(Array.from(step1.session.marks.values())).toContain('correctSoFar');
    
    let step2 = playSession(step1.session, ['on:48@150']);
    expect(step2.session.index).toBe(0);
    
    let step3 = playSession(step2.session, ['on:64@200']);
    expect(step3.session.index).toBe(0);

    let step4 = playSession(step3.session, ['on:72@250']);
  ...
  it('every expected note needs a fresh press, already down blocks', () => {
    ...
    let step1 = playSession(session, ['on:62@50']);
    let step2 = playSession(step1.session, ['on:60@100']);
    ...
    let step3 = playSession(step2.session, ['off:62@150']);
    expect(step3.session.phase).toBe('waiting'); // unblocked
    
    let step4 = playSession(step3.session, ['on:62@200']);
  ...
  it('played-along keys are marked but not judged', () => {
    ...
    let step1 = playSession(session, [`on:${acc?.key}@100`]);
  ...
  it('releasing a long note early never blocks', () => {
    ...
    let step1 = playSession(session, ['on:60@100']);
    expect(step1.session.index).toBe(1);
    
    let step2 = playSession(step1.session, ['off:60@150']);
  ...
  it('extra key when blocked', () => {
    ...
    let step1 = playSession(session, ['on:62@50', 'on:60@100']);
    expect(step1.session.phase).toBe('blocked');
    
    let step2 = playSession(step1.session, ['on:64@150']);
  ...
  it('sustain ignored for judging', () => {
    ...
    let step1 = playSession(session, ['sus:down@50']);
