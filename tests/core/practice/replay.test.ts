...
describe('replay (snapshots)', () => {
  ...
  function playAll(scoreName: string, inputs: readonly PracticeInput[]) {
    ...
    let session = startSession({ scoreId: 'test', events, startEventIndex: 0, loop: null, accompaniment: false, help: false });
  ...
  it('golden snapshot: scale-c-major-q100', () => {
    ...
    const inputs = buildSequence([
      'on:60@100', 'off:60@150', // C4 correct
      'on:61@200', 'off:61@250', // wrong Pitch
      'on:62@300', 'off:62@350', // D4 correct
      'on:64@400', 'off:64@450', // E4 correct
    ]);
  ...
  it('golden snapshot: chord-basic (using grand-staff)', () => {
    const inputs = buildSequence([
      'on:43@100', 'on:48@120', 'on:64@140', 'on:72@160', // full chord
    ]);
  ...
  it('stripping timeStampMs changes nothing', () => {
    const inputs = buildSequence([
      'on:60@100', 'off:60@150', 
      'on:61@200', 'off:61@250',
      'on:62@300', 'off:62@350',
    ]);
